...
inline returnValue QProblem::setA( Matrix *A_new )
{
	...
	if ( A_new == 0 )
		//std::cout << "A is null but so is nC" << std::endl;
...
inline returnValue QProblem::setA( const real_t* const A_new )
{
	...
	if ( A_new == 0 ){
		//std::cout << "I think A is null" << std::endl;
		return THROWERROR( RET_INVALID_ARGUMENTS );}
	...
	//std::cout << "middle of A setup" << std::endl;
	A->times(1, 1.0, x, nV, 0.0, Ax, nC);
...
{
	//std::cout << "inside LBAsetup" << std::endl;
	uint_t i;
	...
	//std::cout << "done with LBA" << std::endl;
