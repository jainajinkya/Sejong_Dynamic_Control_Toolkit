...
QProblem::QProblem( int_t _nV, int_t _nC, HessianType _hessianType ) : QProblemB( _nV,_hessianType )
{
	...
	{
		_nV = 1;
		//std::cout << "Numvar is one?" << std::endl;
	...
	{
		_nC = 0;
		//std::cout << "I think numconst is not properly set" << std::endl;
		THROWERROR( RET_INVALID_ARGUMENTS );
	}
...
{
	//std::cout << "i am inside initializer" << std::endl;
        int_t i;
	int_t nV = getNV( );
	int_t nC = getNC( );
	...
	if ( guessedBounds != 0 )
	{
		for( i=0; i<nV; ++i )
		{
			if ( guessedBounds->getStatus( i ) == ST_UNDEFINED ){
				//std::cout << " Don't like the bounds" << std::endl;
	...
	if ( guessedConstraints != 0 )
	{
		for( i=0; i<nC; ++i )
			if ( guessedConstraints->getStatus( i ) == ST_UNDEFINED ){
				//std::cout << "Constraints problem" << std::endl;
				return THROWERROR( RET_INVALID_ARGUMENTS );}
	}
							...
							)
{
	...
	//std::cout << 1 << std::endl;
	//std::cout << guessedBounds << std::endl;
	if ( guessedBounds != 0 )
	{
		for( i=0; i<nV; ++i )
		{
			if ( guessedBounds->getStatus( i ) == ST_UNDEFINED ){
				//std::cout << "bounds not defined" << std::endl;
				return THROWERROR( RET_INVALID_ARGUMENTS );}
		}
	}
	//std::cout << 2 << std::endl;
	//std::cout << guessedConstraints << std::endl;
	if ( guessedConstraints != 0 )
	{
		for( i=0; i<nC; ++i )
			if ( guessedConstraints->getStatus( i ) == ST_UNDEFINED ){
				//std::cout << "some constraint not defined" << std::endl;
				return THROWERROR( RET_INVALID_ARGUMENTS );}
	}
	...
	/* exclude these possibilities in order to avoid inconsistencies */
	if ( ( xOpt == 0 ) && ( yOpt != 0 ) && ( ( guessedBounds != 0 ) || ( guessedConstraints != 0 ) ) ){
		//std::cout << " I guess this is where it goes mad" << std::endl;
		return THROWERROR( RET_INVALID_ARGUMENTS );}
	...
	/* 2) Setup QP data. */
	if ( setupQPdata( _H,_g,_A,_lb,_ub,_lbA,_ubA ) != SUCCESSFUL_RETURN ){
		//std::cout << "NO, this is where it goes mad" << std::endl;
									...
									)
{
	...
	if ( ( nC > 0 ) && ( _A == 0 ) )
                //std::cout << "A matrix is NULL while constraint is 1" << std::endl;
		return THROWERROR( RET_INVALID_ARGUMENTS );
...
{
	int_t nC = getNC( );
	//std::cout << "inside set up data function" << std::endl;
	/* 1) Load Hessian matrix as well as lower and upper bounds vectors. */
	if ( QProblemB::setupQPdata( _H,_g,_lb,_ub ) != SUCCESSFUL_RETURN ){
		//std::cout << "problem with qbclass" << std::endl;
		return THROWERROR( RET_INVALID_ARGUMENTS );}
	...
	if ( nC > 0 )
	{
		//std::cout << "setting up constraint bounds" << std::endl;
		/* 2) Setup lower/upper constraints' bounds vector. */
		setLBA( _lbA );
		setUBA( _ubA );
		/* 3) Only load constraint matrix after setting up vectors! */
		//std::cout << _A << std::endl;
		setA( _A );
		//std::cout << "done with A set up also" << std::endl;
	}
	//std::cout << "finished with setupdata" << std::endl;
	return SUCCESSFUL_RETURN;
}
