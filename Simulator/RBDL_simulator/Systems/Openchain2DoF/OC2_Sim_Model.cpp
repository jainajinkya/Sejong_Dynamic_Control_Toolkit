...
OC2_Sim_Model* OC2_Sim_Model::GetOC2_Sim_Model(){
  static OC2_Sim_Model oc3_sim_model_;
  return & oc3_sim_model_;
}

OC2_Sim_Model::OC2_Sim_Model(){
  model_ = new Model();
  ...
  Body link1, link2, j2position, link_ee;
...
void OC2_Sim_Model::UpdateModel(const Vector & q, const Vector & qdot){
...
bool OC2_Sim_Model::getMassInertia(sejong::Matrix & A) {
  A = A_;
  return true;
}

bool OC2_Sim_Model::getGravity(Vector & grav) {
  grav = grav_;
  return true;
}

bool OC2_Sim_Model::getCoriolis(Vector & coriolis) {
  coriolis = coriolis_;
  return true;
}

void OC2_Sim_Model::getFullJacobian(int link_id, sejong::Matrix & J) const {
...
void OC2_Sim_Model::getFullJacobianDot(int link_id, sejong::Matrix & Jdot) const {
...
void OC2_Sim_Model::getPos(int link_id, Vect3 & pos) {
  sejong::Vector q;
...
void OC2_Sim_Model::getVel(int link_id, Vect3 & vel) {
  sejong::Vector q, qdot;
...
unsigned int OC2_Sim_Model::_find_body_idx(int id) const {
  switch(id){
  case LK_SIM_L1:
