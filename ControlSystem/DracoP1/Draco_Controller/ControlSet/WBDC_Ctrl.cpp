...
void WBDC_Ctrl::_body_ctrl2(sejong::Vector &gamma){
    gamma.setZero();
  // A, cori, grav
  _CopyMassGravityCoriolis();
  // Foot (Contact constraint)
  ...
  selected_task[0] = 0;
  selected_task[1] = 0;
  selected_task[2] = 1; // Body x
  selected_task[3] = 1; // Body y
  selected_task[4] = 1; // contact x

  //  B & c
  sejong::Matrix Jtmp;
  sejong::Matrix B_now, B_pre, B_;
  sejong::Vector c_;
  B_.setZero();
  ...
  wbdc_new_.MakeTorque(A_fix_, cori_fix_, grav_fix_, Jc,
                      Icm_, Jcm_, JcmDotQdot_,
                      grav_cm, Sf,
                      B_, x_task, selected_task,
                      c_,
                      tau_min_, tau_max_, cmd_, optimization_results_);
  gamma = cmd_;
