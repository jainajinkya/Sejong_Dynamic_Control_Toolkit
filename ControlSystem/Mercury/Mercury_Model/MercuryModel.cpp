...
MercuryModel* MercuryModel::GetMercuryModel(){
    static MercuryModel draco_model_;
    return & draco_model_;
}
...
void MercuryModel::UpdateModel(const Vector & q, const Vector & qdot){
    UpdateKinematicsCustom(*model_, &q, &qdot, NULL);
    dyn_model_->UpdateDynamics(q, qdot);
    kin_model_->UpdateKinematics(q, qdot);
}

void MercuryModel::getCentroidInertia(sejong::Matrix & Icent){
...
void MercuryModel::getCentroidJacobian(sejong::Matrix & Jcent){
...
void MercuryModel::UpdateKinematics(const Vector & q, const Vector &qdot){
    UpdateKinematicsCustom(*model_, &q, &qdot, NULL);
}

bool MercuryModel::getInverseMassInertia(sejong::Matrix & Ainv) {
    return dyn_model_->getInverseMassInertia(Ainv);
}

bool MercuryModel::getMassInertia(sejong::Matrix & A) {
    return dyn_model_->getMassInertia(A);
}

bool MercuryModel::getGravity(Vector & grav) {
    return dyn_model_->getGravity(grav);
}

bool MercuryModel::getCoriolis(Vector & coriolis) {
    return dyn_model_->getCoriolis(coriolis);
}

void MercuryModel::getFullJacobian(const Vector & q, int link_id, sejong::Matrix & J) const {
...
void MercuryModel::getFullJacobianDot(const Vector & q, const Vector & qdot, int link_id, sejong::Matrix & Jdot) const {
...
void MercuryModel::getPosition(const Vector & q,
...
void MercuryModel::getVelocity(const Vector & q, const Vector &qdot,
...
void MercuryModel::getCoMJacobian(const Vector & q, sejong::Matrix & J){
    J = sejong::Matrix::Zero(3, model_->qdot_size);
    kin_model_->getCoMJacobian(q, J);
}

void MercuryModel::getCoMPosition(const Vector & q, Vect3 & com_pos, bool update){
  com_pos = kin_model_->com_pos_;
    // kin_model_->getCoMPos(q, com_pos, update);
}

void MercuryModel::getCoMVelocity(const Vector & q, const Vector & qdot, Vect3 & com_vel){
    kin_model_->getCoMVel(q, qdot, com_vel);
}
void MercuryModel::getCentroidVelocity(sejong::Vector & centroid_vel){
