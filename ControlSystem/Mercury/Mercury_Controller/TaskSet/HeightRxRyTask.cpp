...
HeightRxRyTask::HeightRxRyTask():WBDC_Task(5),
                                 Kp_(200.0),
                                 Kd_(25.0)
{
  ...
  printf("[HeightRxRy Task] Constructed\n");
}

HeightRxRyTask::~HeightRxRyTask(){}


bool HeightRxRyTask::_UpdateCommand(void* pos_des,
  ...
  // CoM X, Y (Skip)
  // CoM Height
  op_cmd_[2] = acc_des[2] + Kp_ * ((*pos_cmd)[2] - com_pos[2]) + Kd_ * (vel_des[2] - com_vel[2]);
...
bool HeightRxRyTask::_UpdateTaskJacobian(){
...
bool HeightRxRyTask::_UpdateTaskJDotQdot(){
  sejong::Matrix Jdot;
  // model_->getFullJacobianDot(sp_->Q_, sp_->Qdot_, SJLinkID::LK_HeightRxRy, Jdot);
  // TODO
