...
void Test_Ctrl::_GenerateBC_MultiTask_BothHand(){
  //  B & c
  sejong::Matrix Jtmp;
  ...
  // 4th (Right Hand)
  sejong::Matrix J4pre_inv;
  sejong::Matrix J4;
  sejong::Matrix J4dot;
  robot_model_->getFullJacobian(sp_->Q_, LK_leftForearmLink, Jtmp);
  J4 = Jtmp.block(3,0, 3, NUM_QDOT);
  robot_model_->getFullJacobianDot(sp_->Q_, sp_->Qdot_, LK_leftForearmLink, Jtmp);
  J4dot = Jtmp.block(3,0, 3, NUM_QDOT);
  sejong::Matrix J4pre = J4 * N_pre;
  _DynConsistent_Inverse(J4pre, J4pre_inv);
  N_pre = N_pre * (eye - J4pre_inv * J4pre);
  B_now = sejong::Matrix (NUM_QDOT, prev_idx + DIM_LEFT_HAND_POS);
  B_now.block(0,0, NUM_QDOT, prev_idx) = (eye - J4pre_inv * J4) * B_pre;
  B_now.block(0, prev_idx, NUM_QDOT, DIM_LEFT_HAND_POS) = J4pre_inv;
  B_pre = B_now;
  c_ = ((eye - J4pre_inv * J4) * c_ - J4pre_inv * J4dot * sp_->Qdot_);
  prev_idx += DIM_LEFT_HAND_POS;
...
void Test_Ctrl::_multi_task_ctrl_both_hand(sejong::Vector & gamma){
  // Sf
  _GenerateSf();
  ...
  _GenerateBC_MultiTask_BothHand();
  ...
  int st_idx(DIM_RForce);
  // CM Task
  _SetCMTaskCMD_Multi(st_idx, x_task);
  st_idx += DIM_CM;
  // RightHand Task
  _SetRightHandPosTaskCMD(st_idx, x_task);
  st_idx += DIM_RIGHT_HAND_POS;
  // LeftHand Task
  _SetLeftHandPosTaskCMD(st_idx, x_task);
  st_idx += DIM_LEFT_HAND_POS;
  // Joint
  _SetJointTaskCMD_Multi(st_idx, x_task);
