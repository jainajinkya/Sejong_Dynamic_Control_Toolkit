...
namespace RigidBodyDynamics {
  ...
  RBDL_DLLAPI void CalcPointJacobianDot( Model &model,
                                         const Math::VectorNd &Q,
                                         const Math::VectorNd &QDot,
                                         unsigned int body_id,
                                         const Math::Vector3d &pt_position,
                                         Math::MatrixNd &G,
                                         bool update_kinematics ){
    LOG << "-------- " << __func__ << " --------" << std::endl;
    // update the Kinematics if necessary
    ...
    unsigned int ref_body_id = body_id;
    Vector3d ref_point = pt_position;
    // Fixed Body Check
    if (model.IsFixedBodyId(body_id)) {
      unsigned int fbody_id = body_id - model.fixed_body_discriminator;
      ref_body_id = model.mFixedBodies[fbody_id].mMovableParent;
      Vector3d base_coords = CalcBodyToBaseCoordinates(model, Q, body_id, pt_position, false);
      ref_point = CalcBaseToBodyCoordinates(model, Q, ref_body_id, base_coords,false);
    }
    // Body Velocity of Point B
    SpatialTransform T_bn = SpatialTransform(Matrix3d::Identity(3,3), -ref_point);
    SpatialVector vel_b = T_bn.inverse().apply(model.v[ref_body_id]);
    SpatialTransform T_rot_gb (model.X_base[ref_body_id].E.transpose(), Vector3d::Zero(3,1));
    SpatialTransform T_n(model.X_base[ref_body_id]);
    SpatialVector v_gb = model.v[ref_body_id];
    // Linear Velocity is Zero
    v_gb.block(3,0, 3,1).setZero();
    MatrixNd adj_v_gb_mt = Transform2adj_mtx(v_gb);
    // std::cout<<"adj v gb mt: \n"<<adj_v_gb_mt<<std::endl;

    unsigned int j = ref_body_id;
    // e[j] is set to 1 if joint j contributes to the jacobian that we are
    // computing. For all other joints the column will be zero.
    // Note: Does not handle custom joint
    while (j != 0) {
      unsigned int q_index = model.mJoints[j].q_index;
      if (model.mJoints[j].mDoFCount == 1) {
        SpatialTransform T_ni;
        T_ni.E  = (T_n.E * model.X_base[j].E.transpose()).transpose();
        T_ni.r = T_n.E * (model.X_base[j].r - T_n.r);
        SpatialVector ad_v_gb_Vb = adj_v_gb_mt * T_bn.inverse().apply(T_ni.inverse().apply ( model.S[j]));

        // std::cout<<model.S[j]<<std::endl;
        // std::cout<<T_ni<<std::endl;
        // std::cout<<"T bn:\n"<<T_bn<<std::endl;
        // std::cout<<T_bn.inverse().apply(T_ni.inverse().apply ( model.S[j]))<<std::endl;

        // V_ni
        SpatialTransform T_in;
        T_in.E = (model.X_base[j].E * T_n.E.transpose() ).transpose();
        T_in.r = model.X_base[j].E * (T_n.r - model.X_base[j].r);
        SpatialVector V_ni = model.v[j] - T_in.inverse().apply(model.v[ref_body_id]);
        // Ad_bn( Ad_ni(ad_v_ni * S) )
        MatrixNd adj_V_ni = Transform2adj_mtx(V_ni);
        SpatialVector ad_v_ni_S = adj_V_ni * model.S[j];
        SpatialVector Ad_T_bn_adj_V_ni_Si = T_bn.inverse().apply(T_ni.inverse().apply( ad_v_ni_S) );

        G.block(0,q_index, 6, 1) = T_rot_gb.apply(ad_v_gb_Vb + Ad_T_bn_adj_V_ni_Si);
        // std::cout<<"model v:\n"<<model.v[j]<<std::endl;
        // std::cout<< "V_ni: \n"<<V_ni<<std::endl;

      } else if (model.mJoints[j].mDoFCount == 3) {
        SpatialTransform T_ni;
        Matrix3d model_X_E = model.X_base[j].E;
        SpatialTransform Rot_ig(model.X_base[j].E, Vector3d::Zero(3,1));

        T_ni.E  = (T_n.E * model_X_E.transpose()).transpose();
        T_ni.r = T_n.E * (model.X_base[j].r - T_n.r);
        // V_ni
        SpatialTransform T_in;
        T_in.E = (model_X_E * T_n.E.transpose() ).transpose();
        T_in.r = model_X_E * (T_n.r - model.X_base[j].r);
        SpatialVector V_ni = model.v[j] - T_in.inverse().apply(model.v[ref_body_id]);
        MatrixNd adj_V_ni = Transform2adj_mtx(V_ni);
        MatrixNd axis_S_mul = model.multdof3_S[j];

        for(int i(0); i<3; ++i){
          SpatialVector model_S = axis_S_mul.block(0, i, 6, 1);
          SpatialVector ad_v_gb_Vb = adj_v_gb_mt * T_bn.inverse().apply(T_ni.inverse().apply ( model_S));

          SpatialVector ad_v_ni_S = adj_V_ni * model_S;
          SpatialVector Ad_T_bn_adj_V_ni_Si = T_bn.inverse().apply(T_ni.inverse().apply( ad_v_ni_S) );

          G.block(0, q_index + i, 6, 1) = T_rot_gb.apply(ad_v_gb_Vb + Ad_T_bn_adj_V_ni_Si);
        }
      }
      j = model.lambda[j];
    }
