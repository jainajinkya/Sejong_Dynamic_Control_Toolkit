  ...
  namespace Addons {
    ...
    bool construct_model (Model* rbdl_model, ModelPtr urdf_model, bool verbose) {
      ...
      if (root->inertial) {
        ...
        Body dummy (1e-8, Vector3d(0.000001, 0.0, 0.0), Vector3d(0., 0.0, 0.0));

        Body virtual_xyz_body ( dummy );
        Joint virtual_xyz_joint = Joint(JointTypeTranslationXYZ);
        ...
        Joint root_joint = Joint(JointTypeSpherical);
