#ifndef WBDC_2D_VIRTUAL_ACCELERATION_H
#define WBDC_2D_VIRTUAL_ACCELERATION_H
...
template <int DIM_RF, int DIM_SELECTED_TASK, int DIM_TASK>
 ...
 WBDC_2D_vir_acc():
  G(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL, DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL),
    ...
    CE(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL, DIM_CM),
    ce0(DIM_CM),
  ...
  bool MakeTorque(const Eigen::Matrix<double, NUM_QDOT,NUM_QDOT> & A,
    ...
    Eigen::Matrix<double, NUM_VIRTUAL + NUM_ACT_JOINT + NUM_VIRTUAL, DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL> JcT_ABSr_A;
    JcT_ABSr_A.block(0,0, NUM_QDOT, DIM_RF) = -Jc.transpose();
    JcT_ABSr_A.block(0,DIM_RF, NUM_QDOT, DIM_SELECTED_TASK) = A*B*Sr;
    JcT_ABSr_A.block(0,DIM_RF + DIM_SELECTED_TASK, NUM_QDOT, NUM_VIRTUAL) = A.block(0,0, NUM_QDOT, NUM_VIRTUAL);
    ...
    for(int i(0); i< DIM_CM; ++i){
      ...
      for(int j(0); j<NUM_VIRTUAL; ++j){
        CE[j+DIM_RF + DIM_SELECTED_TASK][i] = Icm(i, j);
      }
    ...
    for(int i(0); i < NUM_ACT_JOINT; ++i){
      for(int j(0); j<DIM_RF + DIM_SELECTED_TASK+NUM_VIRTUAL; ++j){
        CI[j][i + DIM_FRICTION] = -JcT_ABSr_A(i+NUM_VIRTUAL,j);
      }
    ...
    for(int i(0); i < NUM_ACT_JOINT; ++i){
      for(int j(0); j<DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL; ++j){
        CI[j][i + DIM_FRICTION + NUM_ACT_JOINT] = JcT_ABSr_A(i+NUM_VIRTUAL,j);
      }
 ...
 private:
  ...
  void _GetSolution(const Eigen::Matrix<double, NUM_QDOT,NUM_QDOT> & A,
    ...
    Eigen::Matrix<double, NUM_VIRTUAL, 1> vir_acc_err;
    // Task error
    int k(0);
    for (int i(0); i<DIM_TASK; ++i){
    ...
    for (int i(0); i<NUM_VIRTUAL; ++i)
      vir_acc_err[i] = x[DIM_RF + DIM_SELECTED_TASK + i];

    sejong::pretty_print((sejong::Vector)err, std::cout, "err");
    sejong::pretty_print((sejong::Vector)vir_acc_err, std::cout, "vir_acc_err");
    sejong::pretty_print((sejong::Vector)Fr, std::cout, "Fr");

    // Qddot
    qddot = B * (x_task + err) + c;
    for(int i(0); i<NUM_VIRTUAL; ++i){
      qddot[i] += vir_acc_err[i];
    }
