...
template <int DIM_RF, int DIM_SELECTED_TASK, int DIM_TASK>
 ...
 WBDC_2D():
  G(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL, DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL),
    g0(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL),
    CE(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL, DIM_CM + NUM_VIRTUAL),
    ce0(DIM_CM + NUM_VIRTUAL),
    CI(DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL, DIM_FRICTION + NUM_ACT_JOINT + NUM_ACT_JOINT),
  ...
  void _SetCost(){
    for(int i(0); i<DIM_RF+DIM_SELECTED_TASK + NUM_VIRTUAL; ++i){
      for(int j(0); j<DIM_RF+DIM_SELECTED_TASK + NUM_VIRTUAL; ++j){
        G[i][j] = 0.;
      }
      g0[i] = 0.;
    }
  ...
  bool MakeTorque(const Eigen::Matrix<double, NUM_QDOT,NUM_QDOT> & A,
    ...
    Eigen::Matrix<double, NUM_VIRTUAL + NUM_ACT_JOINT, DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL> JcT_ABSr_I;
    JcT_ABSr_I.block(0,0, NUM_QDOT, DIM_RF) = -Jc.transpose();
    JcT_ABSr_I.block(0,DIM_RF, NUM_QDOT, DIM_SELECTED_TASK) = A*B*Sr;
    JcT_ABSr_I.block(0,DIM_RF + DIM_SELECTED_TASK, NUM_VIRTUAL, NUM_VIRTUAL) = -Eigen::Matrix<double, NUM_VIRTUAL, NUM_VIRTUAL>::Identity();
    ...
    for (int i(0); i<DIM_CM + NUM_VIRTUAL; ++i){
      for(int j(0); j<DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL; ++j){
        CE[j][i] = 0.;
      }
      ce0[i] = 0.;
    ...
    for(int i(0); i<NUM_VIRTUAL; ++i){
      for(int j(0); j<DIM_RF+DIM_SELECTED_TASK+NUM_VIRTUAL; ++j){
        CE[j][i + DIM_CM] = JcT_ABSr_I(i,j);
      }
      ce0[i + DIM_CM] = ABxt_c_b_g[i];
    }
    std::cout<<CE<<std::endl;
    std::cout<<ce0<<std::endl;

    Eigen::Matrix<double, DIM_CM + NUM_VIRTUAL, DIM_RF + DIM_SELECTED_TASK + NUM_VIRTUAL> Test_Equal;
    Test_Equal.setZero();
    Test_Equal.block(0, 0, DIM_CM, DIM_RF) = -Sf;
    Test_Equal.block(0, DIM_RF, DIM_CM, DIM_SELECTED_TASK) = Icm * Jcm * B * Sr;
    Test_Equal.block(DIM_CM, 0, NUM_VIRTUAL, DIM_RF + DIM_SELECTED_TASK +  NUM_VIRTUAL) = JcT_ABSr_I.block(0,0, DIM_CM, DIM_RF + DIM_SELECTED_TASK +  NUM_VIRTUAL);

    std::cout<<"Test Equal\n";
    Eigen::MatrixXd tmp_mtx = Test_Equal.transpose();
    std::cout<<Test_Equal<<std::endl;
    printf("\n");
    std::cout<<tmp_mtx<<std::endl;
    ...
    for(int i(0); i < NUM_ACT_JOINT; ++i){
      for(int j(0); j<DIM_RF + DIM_SELECTED_TASK; ++j){
        CI[j][i + DIM_FRICTION] = -JcT_ABSr_I(i+NUM_VIRTUAL,j);
      }
    ...
    for(int i(0); i < NUM_ACT_JOINT; ++i){
      for(int j(0); j<DIM_RF + DIM_SELECTED_TASK; ++j){
        CI[j][i + DIM_FRICTION + NUM_ACT_JOINT] = JcT_ABSr_I(i+NUM_VIRTUAL,j);
      }
